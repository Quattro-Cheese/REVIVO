...
- [Introduction (한 줄 소개)](#introduction-한-줄-소개)
- [Background (개발 배경)](#background-개발-배경)
- [Key Features (주요 기능)](#key-features-주요-기능)
  - [1. Real-time Posture Analysis & Feedback (실시간 자세 분석 & 피드백)](#1-real-time-posture-analysis--feedback-실시간-자세-분석--피드백)
  - [2. AI Learning Analytics (AI 학습 분석)](#2-ai-learning-analytics-ai-학습-분석)
...
- [Signal Processing Pipeline (신호 처리 파이프라인)](#signal-processing-pipeline-신호-처리-파이프라인)
- [ML Model Details (ML 모델 상세)](#ml-model-details-ml-모델-상세)
- [Getting Started (실행 방법)](#getting-started-실행-방법)
- [Deployment (배포)](#deployment-배포)
- [Team Members (팀원 소개)](#team-members-팀원-소개)
...
### 1. Real-time Posture Analysis & Feedback (실시간 자세 분석 & 피드백)
...
### 2. AI Learning Analytics (AI 학습 분석)
...
## Signal Processing Pipeline (신호 처리 파이프라인)
...
### Label Generation (레이블 생성 — 비지도 규칙 기반)
...
### 3. CPR Analysis Module (CPR 분석 모듈)
...
### Tasks & Responsibilities (팀원별 상세 수행 역할)

|  |  |  |
|--------|--------|--------|
| 김윤지 | <a href="https://github.com/yooncandooit"><img src="https://github.com/yooncandooit.png" width="100"/></a> | <ul><li>웹 서비스 및 인프라 구축: 풀스택 개발, 환경 세팅, 세션 관리 기능 구현 및 DB 연동</li><li>경량화 AI 적용: RAG 모델 최적화 및 인덱스 재빌드를 통한 서버 성능 개선</li></ul> |
| 정유경 | <a href="https://github.com/ugang04"><img src="https://github.com/ugang04.png" width="100"/></a> | <ul><li>하드웨어 연동: 초음파 센서의 시리얼 통신(Serial Reader) 구현 및 BPM·압박 깊이 계산 로직을 메인 애플리케이션에 연동</li><li>실시간 피드백 시스템: TTS 음성 피드백 기능 개발 및 CPR 피드백 우선순위 로직 수정, 세션 CSV 데이터 저장 추가</li></ul> |
| 김윤기 | <a href="https://github.com/62-62"><img src="https://github.com/62-62.png" width="100"/></a> | <ul><li>데이터 정제 및 보정: 초음파 거리(Depth) 데이터의 정수 변환 및 튀는 값(노이즈) 필터링 알고리즘 설계</li><li>판단 로직 안정화: 압박 해제 임계값(release_threshold) 세부 조정 및 CPR 반복 카운터 로직(RepCounter) 모듈 최적화</li></ul> |
| 안아름 | <a href="https://github.com/arumicube"><img src="https://github.com/arumicube.png" width="100"/></a> | <ul><li>초기 비전·하드웨어 기반 구축: MediaPipe Pose 모듈 세팅 및 연동, 초음파 센서 압박 깊이 측정·LED 피드백 구현, Pose Estimation 로직 작성</li><li>비전 로직 설계: MediaPipe Pose 기반 핵심 커스텀 비전 로직 설계 및 팀 개발 기여 코드 리뷰</li></ul> |
